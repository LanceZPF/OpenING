...
OUTPUT_DIR = './MiniGPT-5_RAG-dev2'  # Define your output directory
...
if __name__ == "__main__":
    ...
    for a_index, a_data in enumerate(io_dir_list):
        ...
        for i in range(gt_out_step):
            ...
            if i == 0:
                input_images = []
                img_path = gtout_image_list[i]
                try:
                    input_image = Image.open(os.path.join(TOTAL_DIR, img_path)).convert("RGB")
                    out_img = expand2square(input_image, (255, 255, 255))
                ...
                system_prompt="Give the following information in text and <Img>ImageContent</Img> format. You will be able to see the images once I provide it to you. Please understanding images and generate images and text."
                utterance = ""

                for i_text in a_data['input_text']:
                    utterance += i_text.replace('<image>','') + "\n"
                    # utterance += i_text.replace('<image>','<Img><ImageHere></Img>') + "\n"

                # for o_text_index in range(i-1):
                #     utterance += gtout_text_list[o_text_index].replace('<image>', '') + "\n"
                #     # utterance += gtout_text_list[o_text_index].replace('<image>', '<Img><ImageHere></Img>') + "\n"

                utterance = system_prompt + f"###Human:{utterance}. The answer is {gtout_text_list[i].replace('<image>', '<Img><ImageHere></Img>')}. Tell me the answer with image. ###Assistant:"
                ...
                if input_image_path:
                    ...
                    for out_img_index in range(i):
                        try:
                            input_image = Image.open(os.path.join(TOTAL_DIR, gtout_image_list[out_img_index])).convert("RGB")
                            out_img = expand2square(input_image, (255, 255, 255))
                            out_img = input_vis_processor(out_img)
                            out_img = out_img.unsqueeze(0).to("cuda:0")
                            input_images.append(out_img)
                ...
                for o_text_index in range(i):
                    utterance += gtout_text_list[o_text_index].replace('<image>', '<Img><ImageHere></Img>') + "\n"
                    # utterance += gtout_text_list[o_text_index].replace('<image>', '<Img><ImageHere></Img>') + "\n"
                utterance = system_prompt + f"###Human:{utterance}. Tell me the next scene with image. ###Assistant:" 
                # utterance = system_prompt + f"###Human:{utterance}. The ground truth is {gtout_text_list[i].replace('<image>', '<Img><ImageHere></Img>')}. Tell me the next scene with image. ###Assistant:" 
