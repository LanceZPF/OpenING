...
OUTPUT_DIR = './seedx_RAG-dev'  # Define your output directory
...
for a_index, a_data in enumerate(tqdm(io_dir_list)):
    ...
    while i_step < gt_out_step:
        ...
        #         image_tokens += BOI_TOKEN + ''.join(IMG_TOKEN.format(int(item)) for item in range(num_img_in_tokens)) + EOI_TOKEN
            ...
            meta_instruction += input_text_list[img_path_i].replace('<image>', '').replace('<BEGIN> ','')
        
        image_path = gtout_image_list[i_step]

        if image_path:
            image_ori = Image.open(os.path.join(TOTAL_DIR, image_path)).convert("RGB")
            source_image = image_ori.resize((1024, 1024))

            image_tensor, patch_pos_tensor = process_anyres_image(source_image, image_transform, grid_pinpoints, base_resolution)
        ...
        instruction = meta_instruction + ' The reference answer is: ' + gtout_text_list[i_step]  + '\n Please repeat the answers.'
        ...
        if output['has_img_output']:
            ...
            prompt = generation_prompt.format_map({'instruction': image_tokens + gtout_text_list[i_step]})
