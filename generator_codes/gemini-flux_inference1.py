...
OUTPUT_DIR = 'E:\\Users\Lance\\BaiduNetdiskDownload\\Gemini1.5+Flux_dumb-dev'  # Define your output directory
...
def get_gpt4answer(input_text, input_images):
    ...
    payload = {"model": "gemini-1.5-pro", "messages": my_message}
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.meta_path, '..\\codes\\few-shot_generation_system.txt'), 'r') as f:
    ...
    for a_index, a_data in tqdm(enumerate(io_dir_list)):
        ...
        if len(output_steps) > gt_out_step:
            output_steps = [output.replace('<IMG>','')+' <IMG>']
