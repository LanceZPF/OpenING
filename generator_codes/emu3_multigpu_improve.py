...
def worker(gpu_id, io_dir_list, real_data_list):
    ...
    for a_index, a_data in enumerate(tqdm(io_dir_list)):
        ...
        while i_step < gt_out_step:
            ...
            # Split the output text into segments
            temp_text_list = output_text.split('. ')
            temp_text_list = [text.strip() + '. ' for text in temp_text_list if text.strip() and len(text.strip()) >= 2]
            
            # Calculate remaining outputs needed
            remaining_outputs = gt_out_step - len(out_text_list)
            
            if len(temp_text_list) > remaining_outputs:
                # Calculate how many segments to combine per output
                segments_per_output = len(temp_text_list) // remaining_outputs
                remainder = len(temp_text_list) % remaining_outputs
            ...
            for text_segment in split_text_list:
                prompt = [f"The prompt for this generation is: {text_segment} The context of this task is: {instruction.replace('[<IMG_PLH>]','')}" + POSITIVE_PROMPT]
                ...
                out_text_list.append(text_segment)
                i_step += 1
