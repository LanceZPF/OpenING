...
OUTPUT_DIR = './Emu2-q_RAG-dev'  # Define your output directory
...
t2i_pipe.vae.to("cuda:1")
t2i_pipe.unet.to("cuda:1")

if device_count >= 2:
    t2i_pipe.multimodal_encoder.to("cuda:2")
    t2i_pipe.safety_checker.to("cuda:2")
else:
    # If only one GPU is available, put everything on GPU 0
    t2i_pipe.multimodal_encoder.to("cuda:1")
    t2i_pipe.safety_checker.to("cuda:1")
    ...
    while i_step < gt_out_step:
        ...
        for out_i in range(i_step):
            if len(input_images) < 2:
                image = Image.open(os.path.join(TOTAL_DIR,gtout_image_list[out_i])).convert('RGB')
                input_images.append(image)
