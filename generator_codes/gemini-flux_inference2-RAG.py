...
def generate_image(a_real_data, input_text_list, output_text, i_step, input_images=[]):
...
if __name__ == '__main__':
    ...
    for a_index, a_data in tqdm(enumerate(io_dir_list), total=len(io_dir_list)):
        ...
        input_images = []
        img_path = os.path.join(TOTAL_DIR, gtout_image_list[0])
        if os.path.join(img_path) and os.path.exists(img_path):
            input_images.append(encode_image(os.path.join(TOTAL_DIR, img_path)))
        else:
            for img_path in gtout_image_list:
                if img_path:
                    input_images.append(encode_image(os.path.join(TOTAL_DIR, img_path)))
                    break
        ...
        for i_step in range(gen_img_num):
            saved_path = generate_image(real_data_list[a_index], input_text_list, gtout_text_list[i_step], i_step, input_images=input_images)
            if saved_path:
